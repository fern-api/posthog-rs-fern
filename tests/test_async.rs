...
mod async_batching_tests {
    ...
    async fn test_concurrent_captures() {
        // Test that multiple concurrent tasks can safely capture events
        use std::sync::Arc;
        ...
        // Spawn 10 concurrent tasks, each sending 10 events (100 total)
        let mut handles = vec![];
        for task_id in 0..10 {
            let client = Arc::clone(&client);
            let handle = tokio::spawn(async move {
                for i in 0..10 {
                    let distinct_id = format!("task_{}_event_{}", task_id, i);
